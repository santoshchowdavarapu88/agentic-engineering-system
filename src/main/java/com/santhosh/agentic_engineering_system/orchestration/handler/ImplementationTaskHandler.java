...
import com.santhosh.agentic_engineering_system.agent.ImplementationAgent;
...
public class ImplementationTaskHandler implements WorkflowTaskHandler {
    private final ImplementationAgent agent;
    @Override public TaskType supports() { return TaskType.IMPLEMENTATION; }
    @Override public TaskExecutionResult execute(EngineeringWorkflow workflow, WorkflowTask task) {
        ...
        return TaskExecutionResult.of(WorkflowContextKeys.IMPLEMENTATION_PATCH, agent.generate(plan, repository));
    }
}
